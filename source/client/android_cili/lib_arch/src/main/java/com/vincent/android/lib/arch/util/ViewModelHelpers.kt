...
package com.vincent.android.lib.arch.util
...
/**
 * 两个参数的ViewModel对应的ViewModelFactory
 */
fun <T : ViewModel, A1, A2> doubleArgsViewModelFactory(constructor: (A1, A2) -> T):
            (A1, A2) -> ViewModelProvider.NewInstanceFactory {
...
class ExampleViewModel(val id: String) : ViewModel() {

    class ViewModelFactory(private val id: String) : ViewModelProvider.Factory {

        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            return ExampleViewModel(id) as T
        }

    }
}

fun testGenerateViewModel(store: ViewModelStore) {

    val exampleViewModel1 = ViewModelProvider(
        store,
        ExampleViewModel.ViewModelFactory("id_100000")
    )[ExampleViewModel::class.java]



    val vmFactory = singleArgViewModelFactory(::ExampleViewModel)
    val exampleViewModel2  = vmFactory("id_100001")
}